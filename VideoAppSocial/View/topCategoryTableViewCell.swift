...
func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
    ...
    let imageUrl = ["https://picsum.photos/200/300", "https://picsum.photos/200/300" , "https://picsum.photos/200/300", "https://picsum.photos/200/300", "https://picsum.photos/200/300"]
    let labelName = ["Shoes","Mobile","Home","Electronics","Other"]
    let imageurldata = imageUrl[indexPath.row]
    let options = labelName[indexPath.row]
    collCell.idNamelbl.text = options
    
     
    let imageURL = URL(string: imageurldata)!
    let data = try? Data(contentsOf: imageURL)
    collCell.idImg.image = UIImage(data: data!)
    return collCell
